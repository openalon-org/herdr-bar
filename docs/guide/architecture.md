...
`FocusRaiser` is isolated from protocol logic. A **row** click is:
...
A named **session-chip arrow** skips 1–2 and only calls `raiseHost` for that session, so the TUI keeps its last selected pane.
